...
extension LiquidMenuButtons {
    ...
    private func LiquidMenu() -> some View {
        ZStack {
            ...
            MinusButton().offset(offsetThree).blendMode(.softLight).opacity(isCollapsed ? 1 : 0)
            
            TrashButton().offset(offsetFive).blendMode(.softLight).opacity(isCollapsed ? 1 : 0)
            PersonButton().offset(offsetSix).blendMode(.softLight).opacity(isCollapsed ? 1 : 0)
            PersonButton().offset(offsetSeven).blendMode(.softLight).opacity(isCollapsed ? 1 : 0)
            PersonButton().offset(offsetEight).blendMode(.softLight).opacity(isCollapsed ? 1 : 0)
        }

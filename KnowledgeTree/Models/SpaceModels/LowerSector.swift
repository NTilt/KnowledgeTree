...
struct LowerSector {
    ...
    init(_ rootSquare: Square) {
        ...
        lowerSideDots.append(rootSquare.getRightLowCorner())
        lowerSideDots.append(contentsOf: rootSquare.getLowSideCoordinates())
    ...
    func getCentersFromLevel(lvl level: Int) -> [Center] {
        ...
        if let rootSquare = squares.first {
            ...
            let rightTopSquareCenter: (x: Float, y: Float) = (x: centerX + Float(level) * lengthSide, y: centerY + lengthSide)
            let leftBotSquareCenter: (x: Float, y: Float) = (x: centerX, y: (centerY + lengthSide) + Float(level) * lengthSide)
            var currentCenter: (x: Float, y: Float) = (x: rightTopSquareCenter.x, y: rightTopSquareCenter.y)
            for y in stride(from: currentCenter.y, through: leftBotSquareCenter.y, by: lengthSide) {
                currentCenter = (x: currentCenter.x, y: y)
                outputCenters.append(Center(coordinates: currentCenter))
            }
            for x in stride(from: currentCenter.x - lengthSide, through: leftBotSquareCenter.x, by: lengthSide) {
        ...
        for center in centers {
            if let square = getSquareFromCenter(from: center) {
                if square.getRightLowCorner().x == maxX {
                    // TODO: Исправить правильность добавления
                    allFreeDots.append(contentsOf: square.getRightSideCoordinates())
                    allFreeDots.append(square.getRightLowCorner())
                }
                if square.getRightLowCorner().y == maxY {
                    // TODO: Исправить правильность добавления
                    allFreeDots.append(contentsOf: square.getLowSideCoordinates())
                    allFreeDots.append(square.getRightLowCorner())
                }
            }
        }

...
struct LeftSector {
    ...
    init(_ rootSquare: Square) {
        ...
        lowerSideDots.append(rootSquare.getLeftLowCorner())
        lowerSideDots.append(contentsOf: rootSquare.getLeftSideCoordinates())
    ...
    func getCentersFromLevel(lvl level: Int) -> [Center] {
        ...
        if let rootSquare = squares.first {
            ...
            let rightBotSquareCenter: (x: Float, y: Float) = (x: centerX - lengthSide, y: centerY + Float(level) * lengthSide)
            let leftTopSquareCenter: (x: Float, y: Float) = (x: (centerX - lengthSide) - Float(level) * lengthSide, y: centerY)
            var currentCenter: (x: Float, y: Float) = (x: rightBotSquareCenter.x, y: rightBotSquareCenter.y)
            for x in stride(from: currentCenter.x, through: leftTopSquareCenter.x, by: -lengthSide) {
                currentCenter = (x: x, y: currentCenter.y)
                outputCenters.append(Center(coordinates: currentCenter))
            }
            for y in stride(from: currentCenter.y - lengthSide, through: leftTopSquareCenter.y, by: -lengthSide) {
    ...
    func getAllFreeDots(from centers: [Center]) -> [Location] {
        ...
        for center in centers {
            if let square = getSquareFromCenter(from: center) {
                if square.getLeftLowCorner().y == maxY {
                    allFreeDots.append(contentsOf: square.getLowSideCoordinates())
                    allFreeDots.append(square.getLeftLowCorner())
                }
            }
        }
        for center in centers {
            if let square = getSquareFromCenter(from: center) {
                if square.getLeftLowCorner().x == minX {
                    allFreeDots.append(square.getLeftLowCorner())
